...
# The __init__ initializes the Pacman's position (abs_x, abs_y) based on row and column values, along with animation-related attributes.
# The _import_character_assets() load Pacman's animations. The status initializes Pacman's status, which is idle as default, the life count (life), and the score (pac_score).
# The _import_character_assets() method imports Pacman's animation sprites from specified directories (assets/pac/) for various actions (up, down, left, right, idle, power_up).
# The walls_collide_list checks if Pacman collides with walls after a specified movement (x, y).
# The move_to_start_pos() resets Pacman's position to its initial coordinates. 
# The animate(pressed_key, walls_collide_list) manages Pacman's animation based on pressed keys and collision detection with walls, then updates Pacman's status and position based on movement direction and collisions.
# In the update() method, the immune manages Pacman's immunity state based on a timer (immune_time) and updates Pacman's position rect based on its image.